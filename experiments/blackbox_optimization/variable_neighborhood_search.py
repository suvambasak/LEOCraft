'''
Variable Neighborhood Search (VNS) for LEO constellation design
Optimizing the design parameters of LEO constellation using VNS
with domain knowledge (heuristics in use) and without domain knowledge (heuristics not in use)
'''
...
class VariableNeighborhoodSearchWithDomainKnowledge:
    '''
...
class VariableNeighborhoodSearchWithoutDomainKnowledge:
    '''
    Optimizes angle of elevation (e), Altitude (h) and inclination (i) with Best First Search (Informed Search)
    ...
    def __init__(self, e: float, h: float, i: float, oxn: int, p: float) -> None:
        '''
        Starting point of the LEO parameters
    ...
    def _generate_moves(self) -> None:
        ...
        # Total 3D: e, h, i
        for num_of_direction in range(1, 5):
            for direction_indices in itertools.combinations(
                [0, 1, 2, 3, 4], num_of_direction
            ):
                # print(direction_indices)
    ...
    def _hash(self, params: list) -> str:
        ...
        str
            Hash of LEO params
        '''
        return f'{params[0]}_{params[2]}_{params[1]}_{params[3]}_{params[4]}'
    ...
    def _generate_next_unique_states(self) -> list[list[float]]:
        ...
        for move in self._moves:
            ...
            # OXN index should be int
            forward_state[3] = int(forward_state[3])
            backward_state[3] = int(backward_state[3])
    ...
    def _dict_to_tuple(self, performance_metrics: dict[str, float | int]) -> list[float]:
        ...
        '''
        id = None
        for i, pair in enumerate(OXN):
            if pair == (performance_metrics.get('S0_o'), performance_metrics.get('S0_n')):
                id = i
                break
    ...
    def _simulate_states(self, states: list[list[float]]) -> None:
        ...
        for state in states:
            ...
            leo_con.add_shells(PlusGridShell(
                ...
                orbits=OXN[state[3]][0],
                sat_per_orbit=OXN[state[3]][1],

                phase_offset=state[4],
...
if __name__ == '__main__':
    ...
    bfs = VariableNeighborhoodSearchWithoutDomainKnowledge(
        ...
        oxn=random.randint(0, len(OXN)-1),
