...
def without_domain_knowledge() -> None:

    def _cost_without_domain_knowledge(x: list) -> float:
        '''
    ...
    def _without_domain_knowledge_results(result: list) -> None:
        print('='*99)
        print(f'''Throughput: {
              _cost_without_domain_knowledge(result) * -1} Gbps''')
        print(f'''
            o , n = {OXN[round(result[3])]}
            h = {round(result[1], 1)}
            i = {round(result[2], 1)}
            e = {round(result[0], 1)}
            p = {round(result[4], 2)}
    ...
    initial = [
        ...
        random.randint(ALTITUDE_LB_KM, ALTITUDE_UB_KM),
        random.uniform(30.0, 90.0),
        random.randint(0, len(OXN)-1),
        random.randint(0, 50)
    ]

    _, pos_best_g = pso_simple.minimize(
        _cost_without_domain_knowledge,
        initial, bounds,
        num_particles=NUM_PARTICLES, maxiter=MAXITER, verbose=True
    )
    _without_domain_knowledge_results(pos_best_g)


def with_domain_knowledge() -> None:

    def _cost_with_domain_knowledge(x: list) -> float:
        '''
        Standard tuple of LEO params
        - ['0', '1', '2']
        - ['e', 'h', 'i']
    ...
    def _with_domain_knowledge_results(result: list) -> None:
        print('='*99)
        print(f'''Throughput: {
            _cost_with_domain_knowledge(result) * -1} Gbps''')
        print(f'''
            o , n = {OXN[-1]}
            h = {round(result[1], 1)}
            i = {round(result[2], 1)}
    ...
    initial = [10.0, random.randint(ALTITUDE_LB_KM, ALTITUDE_UB_KM), 30.0]
    ...
    _with_domain_knowledge_results(pos_best_g)
